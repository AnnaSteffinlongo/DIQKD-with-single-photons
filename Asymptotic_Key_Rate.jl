...
function keyrate(ρ, ηD_AB::Float64, X::Vector{Float64}, q::Float64, L, ηL_fix::Bool; squeezing::Bool = true)
    ...
    if ηL_fix
        if QD
            heralding_rate = 2 * R * T * (1-T) * ηD_C * 10^(-0.2*L/20) 
        else
            heralding_rate = 2 * ξ^4 * ηSPDC^2 * T * (1-T) * ηD_C * 10^(-0.2*L/20) * R
        end
...
R_QD = 75e6; g = 1*10^-2; V_QD = 0.975                      # QD source params
...
folder = "results/"
file_name = sq ? "CHSH_squeezing_test.csv" : "CHSH_dispacement.csv"
...
g_str = string(round(g, digits=3))
xi_str = string(round(ξ, digits=3))
eta_str = string(eta_val)
# Save results
file_name = "asymptotic_L_QD_etaL$(eta_val)_g$(g_str)_V$(V_str)_T$(T_str).csv"
